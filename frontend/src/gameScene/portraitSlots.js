...
class PortraitSlots {
     ...
     */
    setupEventListeners() {
        const canvas = this.interactionElement ?? document.querySelector('canvas');
        if (!canvas) {
            console.warn('PortraitSlots could not find a canvas to attach event listeners.');
            return;
        }
     ...
     */
    updateMouse(event) {
        const canvas = this.canvas ?? this.interactionElement ?? document.querySelector('canvas');
        if (!canvas) {
            return;
        }
