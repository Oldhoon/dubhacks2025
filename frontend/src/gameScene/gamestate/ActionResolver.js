import { getCharacterForType, parseTypeDeclaration, canPerformOperation } from './TypeMappings.js';
...
export default class ActionResolver {
    ...
    registerDefaultHandlers() {
        // Declaration: spawn character for type
        this.registerHandler('declare', this.handleDeclaration.bind(this));

        // Assignment: place entity at position
        this.registerHandler('assign', this.handleAssignment.bind(this));

        // Reference: create pointer connection
        this.registerHandler('reference', this.handleReference.bind(this));

        // Dereference write: pointer write operation
        this.registerHandler('deref_write', this.handleDereferenceWrite.bind(this));
        this.registerHandler('deref', this.handleDereference.bind(this)); // Legacy support

        // Dereference read: copy pointed value
        this.registerHandler('deref_read', this.handleDereferenceRead.bind(this));

        // Array access: shuffle/swap array elements
        this.registerHandler('array_access', this.handleArrayAccess.bind(this));
    ...
    /**
     * Handle declaration operation
     * Example: int x; (spawn Necromancer for int type)
     */
    async handleDeclaration(action, codeLine) {
        const { operands } = codeLine;
        const { entityId, target, cType } = action.params;

        if (!operands || !operands.type || !operands.variable) {
            return {
                success: false,
                message: 'Declaration requires type and variable name'
            };
        }

        // Determine character type from C type
        const characterType = cType || getCharacterForType(operands.type);

        // Create entity for this variable
        const entity = this.worldState.addEntity({
            id: entityId || `${operands.variable}-entity`,
            type: characterType,
            position: target || null,
            variables: {
                value: operands.initialValue ?? null
            },
            metadata: {
                cType: operands.type,
                variableName: operands.variable
            }
        });

        // Bind variable
        this.worldState.setVariable(operands.variable, operands.initialValue ?? null, operands.type);
        this.worldState.bindVariable(operands.variable, entity.id);

        return {
            success: true,
            message: `Declared ${operands.type} ${operands.variable} with ${characterType}`
        };
    }

    /**
     * Handle reference operation
     * Example: int *p = &x; (Catapult connects to target variable)
     */
    async handleReference(action, codeLine) {
        const { operands } = codeLine;
        const { pointerEntityId, targetEntityId, target } = action.params;

        if (!operands || !operands.pointer || !operands.target) {
            return {
                success: false,
                message: 'Reference requires pointer and target'
            };
        }

        // Create catapult entity for the pointer
        const catapultEntity = this.worldState.addEntity({
            id: pointerEntityId || `${operands.pointer}-entity`,
            type: 'catapult',
            position: target || null,
            variables: {
                pointsTo: targetEntityId || operands.target
            },
            metadata: {
                cType: 'pointer',
                variableName: operands.pointer,
                targetVariable: operands.target
            }
        });

        // Create pointer variable that points to target
        const targetVar = this.worldState.getVariable(operands.target);
        if (targetVar) {
            this.worldState.setVariable(
                operands.pointer,
                targetVar.boundTo, // Store entity ID of target
                'pointer'
            );
            this.worldState.bindVariable(operands.pointer, catapultEntity.id);
        }

        return {
            success: true,
            message: `Created pointer ${operands.pointer} -> ${operands.target}`
        };
    }

    /**
     * Handle dereference write operation
     * Example: *p = 4; (Catapult fires to update linked variable)
     */
    async handleDereferenceWrite(action, codeLine) {
        const { operands } = codeLine;
        ...
        if (!operands || !operands.pointer) {
            return {
                success: false,
                message: 'Dereference write requires pointer'
            };
        }
        ...
        if (!pointerVar || !pointerVar.boundTo) {
            return {
                success: false,
                message: 'Pointer not bound to catapult entity'
            };
        }

        // Get catapult entity
        const catapultEntity = this.worldState.getEntity(pointerVar.boundTo);
        if (!catapultEntity) {
            return {
                success: false,
                message: 'Catapult entity not found'
            };
        }

        // Get target entity that the catapult points to
        const targetEntityId = catapultEntity.variables?.pointsTo;
        if (!targetEntityId) {
            return {
                success: false,
                message: 'Catapult not connected to target'
            };
        }

        const targetEntity = this.worldState.getEntity(targetEntityId);
        if (!targetEntity) {
            return {
                success: false,
                message: 'Target entity not found'
        ...
        // Update target entity's value (catapult fires!)
        targetEntity.variables.value = value ?? operands.value;

        // Update the variable that target entity represents
        const targetVarName = targetEntity.metadata?.variableName;
        if (targetVarName) {
            this.worldState.setVariable(targetVarName, targetEntity.variables.value);
        }

        return {
            success: true,
            message: `Dereferenced *${operands.pointer} = ${targetEntity.variables.value}`
        };
    }

    /**
     * Handle dereference read operation
     * Example: int y = *p; (spawn new variable that copies pointed value)
     */
    async handleDereferenceRead(action, codeLine) {
        const { operands } = codeLine;
        const { entityId, target } = action.params;

        if (!operands || !operands.variable || !operands.pointer) {
            return {
                success: false,
                message: 'Dereference read requires variable and pointer'
            };
        }
        ...
        if (!targetEntityId) {
            return {
                success: false,
                message: 'Pointer not connected to target'
            };
        }

        // Get pointed-to entity's value
        const targetEntity = this.worldState.getEntity(targetEntityId);
        const copiedValue = targetEntity?.variables?.value;

        // Determine character type for new variable
        const characterType = getCharacterForType(operands.type || 'int');

        // Create new entity with copied value
        const newEntity = this.worldState.addEntity({
            id: entityId || `${operands.variable}-entity`,
            type: characterType,
            position: target || null,
            variables: {
            ...
            metadata: {
                cType: operands.type || 'int',
                variableName: operands.variable
            }
        });

        // Create variable
        this.worldState.setVariable(operands.variable, copiedValue);
        this.worldState.bindVariable(operands.variable, newEntity.id);

        return {
            success: true,
            message: `Created ${operands.variable} = *${operands.pointer} (value: ${copiedValue})`
        };
    }

    /**
     * Handle array access operation
     * Example: trees[1] = trees[3]; (shuffle/swap field tiles)
     */
    async handleArrayAccess(action, codeLine) {
        const { operands } = codeLine;
        const { sourceIndex, destIndex } = action.params;

        if (!operands || !operands.array) {
            return {
                success: false,
                message: 'Array access requires array name'
            };
        }

        // Get array variable
        const arrayVar = this.worldState.getVariable(operands.array);
        if (!arrayVar || !arrayVar.boundTo) {
            return {
                success: false,
        ...
        // Get field entity
        const fieldEntity = this.worldState.getEntity(arrayVar.boundTo);
        if (!fieldEntity) {
            return {
                success: false,
                message: 'Field entity not found'
            };
        }

        // Array data stored in entity variables
        if (!fieldEntity.variables.elements) {
            fieldEntity.variables.elements = [];
        }

        const elements = fieldEntity.variables.elements;
        const srcIdx = sourceIndex ?? operands.sourceIndex;
        const dstIdx = destIndex ?? operands.destIndex;

        if (srcIdx === undefined || dstIdx === undefined) {
            return {
                success: false,
                message: 'Source and destination indices required'
            };
        }

        // Perform array operation (copy or swap)
        if (operands.operation === 'swap') {
            // Swap elements
            const temp = elements[srcIdx];
            elements[srcIdx] = elements[dstIdx];
            elements[dstIdx] = temp;
        } else {
            // Copy/assign
            elements[dstIdx] = elements[srcIdx];
        }

        return {
            success: true,
            message: `Array ${operands.array}[${dstIdx}] = ${elements[dstIdx]}`
        };
    }
